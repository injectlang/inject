...
//
// The simplest use of a ContextFunc is to give custom functions access to the
...
// DecodeCustomFunctions looks for blocks of type "custom_function" in the given body
...
func DecodeCustomFunctions(parser *hclparse.Parser, src []byte, filename string, context ContextFunc) (funcs map[string]function.Function, remain hcl.Body, diags hcl.Diagnostics) {
	return decodeCustomFunctions(parser, src, filename, context)
}
